...
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ExtendedStdString buf(buf_size);
...
TEST(StringPieceTest,ConstructTest2)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_EQ(content_size,str.size());
            EXPECT_EQ(0,memcmp(str.const_raw_data(),content.data(),content_size));
            EXPECT_EQ(buf.cdata(),str.const_raw_data());
        }
    }
}
...
TEST(StringPieceTest,ConstructTest3)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_TRUE(str.empty());
            EXPECT_EQ(content_size,str1.size());
            EXPECT_EQ(0,memcmp(str1.const_raw_data(),content.data(),content_size));
            EXPECT_EQ(buf.cdata(),str1.const_raw_data());
        }
    }
}
...
TEST(StringPieceTest,ConstructTest4)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_EQ(content_size,str.size());
            EXPECT_EQ(0,memcmp(str.const_raw_data(),content.data(),content_size));
            EXPECT_EQ(buf.const_raw_data(),str.const_raw_data());
        }
    }
}
...
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ExtendedStdString buf1(buf_size);
        ExtendedStdString buf2(buf_size);

        for (size_t content_size = 0; content_size <= buf_size; ++content_size){
            std::string content = CreateRandomStdString(content_size);

            StringPiece str(buf1.data(),buf_size,content.data());

            for (size_t pos = 0; pos < str.size(); ++pos) {
                {
                    StringPiece str2(buf2.data(),buf_size,str,pos,str.npos);
                    EXPECT_EQ(buf2.const_raw_data(),str2.const_raw_data());
                    EXPECT_EQ(str.size() - pos,str2.size());
                    EXPECT_EQ(0,memcmp(str2.const_raw_data(),str.const_raw_data() + pos,str2.size()));
                }
    ...
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            for (size_t idx = 0; idx < str.size(); ++idx)
                EXPECT_EQ(ch,str.const_raw_data()[idx]);
        }
    }
}
...
{
    for (size_t buf1_size = 0; buf1_size <= 2048; ++buf1_size) {
        ExtendedStdString buf1(buf1_size);
        ...
        for (size_t content1_size = 0; content1_size <= buf1_size; ++content1_size) {
            ...
            for (size_t buf2_size = 0; buf2_size <= 2000; ++buf2_size) {
                ExtendedStdString buf2(buf2_size);
                ...
                for (size_t content2_size = 0; content2_size <= buf2_size; ++content2_size) {
                    ...
                    if (str1.size() > str2.capacity()) {
                        EXPECT_ANY_THROW(str2 = str1);
                    } else {
                        str2 = str1;
                        EXPECT_TRUE(IsEqual(content,str2));
                    }
                }
            }
        }
        ...
        for (size_t content1_size = 0; content1_size <= buf1_size; ++content1_size) {
            ...
            for (size_t buf2_size = 0; buf2_size <= 2000; ++buf2_size) {
                ...
                for (size_t content2_size = 0; content2_size <= buf2_size; ++content2_size) {
                    ...
                    if (str1.size() > str2.capacity()) {
                        EXPECT_ANY_THROW(str2 = std::move(str1));
                    } else {
                        str2 = std::move(str1);
                        EXPECT_TRUE(IsEqual(content,str2));
                    }
...
{
    for (size_t content_size = 0; content_size <= 2048; ++content_size) {
        ...
        for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
            ...
            if (content.size() > str.capacity()) {
                EXPECT_ANY_THROW(str = content.data());
            } else {
                str = content.data();
                EXPECT_TRUE(IsEqual(content,str));
            }
        }
    }
}
...
TEST(StringPieceTest,OperatorAssign4)
{
    for (size_t buf_size = 0; buf_size <= 10; ++buf_size) {
        ...
        if (str.capacity() < 1) {
            EXPECT_ANY_THROW(str = ch);
        } else {
            str = ch;
            EXPECT_EQ(1,str.size());
            EXPECT_EQ(ch,*str.const_raw_data());
        }
    }
}
...
TEST(StringPieceTest,BeginEndTest)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_EQ(str.size(),std::distance(str.begin(),str.end()));
...
TEST(StringPieceTest,CBeginCEnd)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_EQ(str.size(),std::distance(str.cbegin(),str.cend()));
...
{
    for (size_t buf_size = 1; buf_size <= 2048; ++buf_size) {
        ExtendedStdString buf(buf_size);
        ...
        for (size_t content_size = 1; content_size <= buf_size; ++content_size) {
            ...
            {
                StringPiece str(buf.raw_data(),buf_size,content.data(),content.size());
                EXPECT_EQ(content.front(),str.front());
                EXPECT_EQ(content.back(),str.back());
                EXPECT_EQ(content.front(),str.cfront());
                EXPECT_EQ(content.back(),str.cback());
            }
            {
                const StringPiece str(buf.raw_data(),buf_size,content.data(),content.size());
                EXPECT_EQ(content.front(),str.front());
...
TEST(StringPieceTest,PushBack)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_ANY_THROW(str.push_back('c'););
        }
    }
}
...
TEST(StringPieceTest,Capacity)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        EXPECT_EQ(buf_size,str.capacity());
    }
}
    ...
    for (size_t buf_size = 0;buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            for (size_t new_size = 0; new_size <= buf_size; ++new_size) {
                ...
                EXPECT_TRUE(IsEqual(content,str));
                EXPECT_ANY_THROW(str.resize(buf_size + new_size + 1,'x'));
            }
        }
    }
}
...
TEST(StringPieceTest,Reserve)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t new_capacity = 0; new_capacity <= buf_size * 2; ++new_capacity) {
            if (new_capacity > buf_size) {
                EXPECT_ANY_THROW(str.reserve(new_capacity));
            } else {
                str.reserve(new_capacity);
                EXPECT_GE(str.capacity(),new_capacity);
            }
        }
    }
}
...
TEST(StringPieceTest,OperatorBracketWrite)
{
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            ...
            EXPECT_TRUE(IsEqual(content2,str));
        }
    }
}
...
TEST(StringPieceTest,OperatorPlusEqual)
{
    ...
    str += "H";
    EXPECT_TRUE(IsEqual(std::string("H"),str));
    str += 'c';
    EXPECT_TRUE(IsEqual(std::string("Hc"),str));

    char buf2[32];
    StringPiece str2(buf2,sizeof(buf2));
    str2 += str;
    EXPECT_TRUE(IsEqual(std::string("Hc"),str2));
}
...
{
    {
        ...
        str2.append(str1);
        EXPECT_TRUE(IsEqual(std::string("HelloWorld"),str2));

        str2.append(str1,3,str1.npos);
        EXPECT_TRUE(IsEqual("HelloWorldloWorld",str2));

        str2.append(str1,3,2);
        EXPECT_TRUE(IsEqual("HelloWorldloWorldlo",str2));
    }
...

    {
        ...
        EXPECT_TRUE(IsEqual("HelloWorld",str));
    }
}
    ...
    for (size_t buf_size = 0; buf_size <= 2048; ++buf_size) {
        ...
        for (size_t content_size = 0; content_size <= buf_size; ++content_size) {
            for (size_t append_size = 0; append_size <= buf_size - content_size; ++append_size) {
                ...
                EXPECT_ANY_THROW(str.append(str.capacity() - str.size() + append_size + 1,'x'));
            }
        }
    }
}
...
STRING_PIECE_TEST(Append3)
{
...

    {
        ...
        EXPECT_ANY_THROW(str.append(append_content,sizeof(append_content)));
    }
}
...
STRING_PIECE_TEST(Append4)
{
    ...
    for (size_t buf_size = 0; buf_size <= 512; ++buf_size) {
        ExtendedStdString buf(buf_size);
...
STRING_PIECE_TEST(Assign1)
{
    ...
    for (size_t buf_size = 0; buf_size <= 512; ++buf_size) {
        ExtendedStdString buffer(buf_size);
...
STRING_PIECE_TEST(Insert)
{
...

    {
        ...
        StringPiece str(buf,sizeof(buf));
        char append_content[18];
        EXPECT_ANY_THROW(str.insert(0,append_content,sizeof(append_content)));
    }
}
...
STRING_PIECE_TEST(Insert2)
{
...

    {
        char buf[16];
        StringPiece str(buf,sizeof(buf));
        EXPECT_ANY_THROW(str.insert(0,sizeof(buf) + 1,'x'));
    }
}
